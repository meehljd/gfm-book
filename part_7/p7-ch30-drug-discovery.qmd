...
### Systematic Target Prioritization with Open Targets {#sec-ch30-opentargets}

The **Open Targets Platform** (platform.opentargets.org) exemplifies systematic target prioritization by aggregating evidence across multiple domains into target-disease association scores [@ochoa_open_2023]. Rather than building ad hoc evidence integration for each therapeutic program, Open Targets provides a standardized framework that foundation model predictions can complement and extend.

: Evidence types integrated by the Open Targets Platform for target-disease association scoring. Foundation models can enhance several components, particularly genetic association refinement and functional consequence prediction. {#tbl-opentargets-evidence}

| Evidence Type | Data Sources | What It Captures | FM Enhancement Opportunity |
|---------------|--------------|------------------|----------------------------|
| Genetic associations | GWAS Catalog, UK Biobank, FinnGen | Population-level risk variants | Fine-mapping, VEP scoring |
| Somatic mutations | COSMIC, IntOGen | Cancer driver genes | Driver vs. passenger classification |
| Rare variant burden | Gene burden tests | High-impact coding variants | AlphaMissense pathogenicity |
| Expression | GTEx, Expression Atlas | Tissue-specific regulation | Expression FMs, cell-type specificity |
| Pathways | Reactome | Biological context | Network embedding propagation |
| Literature | Europe PMC text mining | Published associations | N/A |
| Known drugs | ChEMBL | Existing therapeutic context | DTI prediction for novel compounds |

Open Targets Genetics (genetics.opentargets.org) addresses the GWAS-to-gene problem systematically through **locus-to-gene (L2G) scores** [@mountjoy_open_2021]. For each GWAS signal, L2G scores rank candidate genes by integrating fine-mapping posterior probabilities, eQTL colocalization evidence, chromatin interaction data, and distance-based priors. This machine learning approach provides principled starting points for interpreting GWAS hits, though experimental validation remains necessary to confirm causality (see @sec-ch03-fine-mapping for fine-mapping methods and @sec-ch26-gwas-to-genes for causal gene identification).

::: {.callout-important}
## Key Insight: Open Targets as Target Validation Baseline

Before developing novel foundation model-based target identification approaches, researchers should benchmark against Open Targets' systematic scoring. A new method that underperforms this established baseline on retrospective target validation likely has limited practical value, regardless of performance on other metrics. Open Targets also provides a natural integration point: foundation model predictions can feed into the evidence framework rather than replacing it entirely.
:::

The platform has begun incorporating foundation model predictions directly. *AlphaMissense* pathogenicity scores are now integrated into the genetic evidence layer, demonstrating how foundation model outputs can be incorporated into systematic target validation pipelines. This integration provides a template for how other foundation model predictions---regulatory variant effects, network-derived scores, expression-based prioritization---might enter established workflows rather than requiring entirely new infrastructure.
