...
::: {.callout-note title="From Variants to Genes: Locus-to-Gene Scores"}
Even after fine-mapping identifies likely causal variants, connecting noncoding variants to their target genes remains challenging. Most GWAS signals fall in regulatory regions that may affect expression of genes other than the nearest gene through long-range enhancer-promoter interactions.

**Locus-to-gene (L2G) scores** address this GWAS-to-gene problem systematically [@mountjoy_open_2021]. The Open Targets Genetics platform integrates multiple evidence types into a machine learning model that ranks candidate genes at each GWAS locus:

- **Fine-mapping probabilities**: Which variants are most likely causal
- **eQTL colocalization**: Genetic variants affecting expression of nearby genes
- **Chromatin contacts**: Physical interactions between enhancers and promoters (Hi-C, pcHi-C)
- **Distance-based priors**: Proximity to transcription start sites

L2G scores provide principled starting points for interpreting GWAS hits, though they remain computational predictions requiring experimental validation. The concept is explored further in @sec-ch26-gwas-to-genes (causal inference context) and @sec-ch30-opentargets (drug discovery applications).
:::
