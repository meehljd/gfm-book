...
::: {.callout-note}
## Linkage Disequilibrium and Its Consequences

**Linkage disequilibrium (LD)** refers to the non-random association between alleles at different genomic positions. When two variants are in strong LD, knowing the genotype at one position predicts the genotype at the other. This correlation arises because nearby variants on a chromosome are inherited together as **haplotype** blocks; recombination only gradually breaks these associations across generations.

LD has three major consequences for genomic data and modeling:

**Imputation.** Genotyping arrays measure only a sparse subset of variants, but LD enables inference of unmeasured positions. If a measured SNP is in strong LD with an unmeasured variant, the unmeasured genotype can be imputed with high accuracy. Reference panels like 1000 Genomes and the Haplotype Reference Consortium provide the LD structure needed for imputation.

**Association without causation.** GWAS detect associations between genotyped variants and phenotypes, but the associated variant is rarely causal. It is typically a "tag SNP" correlated with the true causal variant through LD. Fine-mapping attempts to disentangle these correlations, but when multiple variants are in near-perfect LD, statistical methods cannot distinguish among them.

**Population differences.** LD patterns differ across ancestries because populations have experienced different recombination histories. A tag SNP that effectively proxies a causal variant in Europeans may be a poor proxy in African-ancestry populations, where shorter LD blocks provide less correlation. This difference is a primary driver of reduced polygenic score portability across populations.
:::
...
The procedure begins by defining a region around an index SNP, often all variants within 1 megabase. Researchers then specify a prior: perhaps at most $K$ variants in the region are causal, with effect sizes drawn from a Gaussian distribution. The observed marginal association statistics (effect sizes and standard errors) together with an LD matrix inform the likelihood of the data under each possible configuration of causal variants. Summation over configurations yields the marginal PIP for each variant.
