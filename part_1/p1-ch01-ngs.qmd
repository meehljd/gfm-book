...
The variant calling pipeline transforms raw sequencing data into high-confidence genotype calls. Sample preparation fragments DNA for either short-read (Illumina) or long-read (PacBio, ONT) sequencing, producing FASTQ files of reads with quality scores. Alignment maps reads to a reference genome, generating BAM/CRAM files that undergo duplicate marking and base quality recalibration. Per-sampl  e variant calling with tools like HaplotypeCaller or DeepVariant produces genomic VCF (gVCF) files. Joint genotyping across the cohort applies population-informed priors, and final filtering separates high-confidence variants from artifacts. File sizes are approximate for 30× whole-genome sequencing.
:::
...
Different data types enable different phasing strategies, each with characteris  tic strengths and resolution. **Read-backed phasing** uses sequencing reads that span multiple heterozygous sites to assign alleles to the same physical molecule. Short reads typically phase variants within tens to hundreds of base pairs, limited by fragment length. Long reads extend this range to tens of kilobases or more, providing direct physical evidence of haplotype structure.

::: {#fig-phasing-compound-het}
![Haplotype phase determines clinical outcome in compound heterozygosity.](../figs/part_1/ch01/02-fig-phasing-compound-het.svg)

A\) Two chromosomes carry the *CFTR* gene with pathogenic variants at two positions. Standard VCF notation (0/1, 0/1) cannot distinguish phase. B\) *Cis* configuration: both variants on the maternal chromosome leave the paternal copy functional; the child is an unaffected carrier. C\) *Trans* configuration: one variant per chromosome eliminates all functional copies, causing cystic fibrosis. Phased VCF notation uses the | delimiter: 0|1, 0|1 indicates cis (same haplotype), while 1|0, 0|1 indicates trans (opposite haplotypes).
:::
...
Three error sources produce distinct failure modes. Sequencing artifacts (instrument noise, library prep, homopolymer slippage) generate false positives. Alignment artifacts (repetitive regions, reference bias) cause both false positives and false negatives. Biological complexity (mosaicism, low allele fractions) primarily causes false negatives when true variants fall below detection thresholds. Training data inherits these systematic errors, creating blind spots that downstream models learn as if they were biological signal.
:::
...
Genomic regions where short-read variant calling systematically fails. Segmental duplications (yellow) including *CYP2D6* on 22q11 create mapping ambiguity. The HLA complex (orange) on 6p21 contains extreme polymorphism. Centromeric regions (gray) lack unique sequence. Tandem repeat loci (*HTT*, *FMR1*) resist short-read genotyping. Lower insets show genome fraction by difficulty class and how long reads resolve ambiguities by spanning entire difficult regions.
:::
...
DeepVariant encodes read evidence as a multi-channel pileup tensor for CNN-based genotype classification. Rows represent reads; columns represent positions around the candidate variant. Channels encode base identity, quality scores, mapping quality, and strand orientation. This example shows a heterozygous SNV with ~50% reads supporting each allele. The CNN outputs posterior probabilities over three genotype classes; the called genotype (0/1) corresponds to the highest probability (0.96).
:::
