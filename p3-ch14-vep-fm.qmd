...
Variant effect prediction (VEP) sits at the intersection of sequence analysis, machine learning, and clinical genetics. This chapter provides comprehensive treatment of VEP methods, from early conservation-based scores through modern foundation models. Earlier chapters introduced specific aspects of this landscape: @sec-vep-classical covered annotation-based methods like CADD and established key concepts including circularity and benchmark design; @sec-cnn examined CNN-based approaches including SpliceAI for splice variants; @sec-protein-lm described protein language models and their application to missense variant scoring. Here we synthesize these threads, examine the full spectrum of VEP approaches, and address cross-cutting challenges in evaluation, calibration, and clinical deployment.

For readers primarily interested in clinical workflows and how VEP tools fit into diagnostic pipelines, @sec-rare-disease provides application-focused perspective. This chapter focuses on the models themselves: what they learn, how they are trained and evaluated, and what their predictions mean.
...
Compared to MSA-based models, large protein LMs provide much broader coverage since they do not require explicit MSAs per protein, capture global patterns such as secondary/tertiary structure and remote homology (@sec-protein-lm), and still require calibration and interpretation to be clinically useful.
...
While zero-shot scores are already competitive, Evo 2 can be fine-tuned or paired with task-specific heads for regulatory prediction (e.g., chromatin accessibility, histone marks), splicing outcomes, and gene expression levels. Such adapters reuse Evo 2's representations as a generic backbone, mirroring how text LLMs are adapted via lightweight fine-tuning and prompting (@sec-fm-principles).
...
The AlphaGenome architecture combines multiple components. Convolutional front-ends extract local motif-level features (akin to models like DeepSEA and Enformer). Long-range attention or state-space blocks propagate information across hundreds of kilobases (@sec-regulatory). Multi-task prediction heads handle different assay types (e.g., ATAC-seq, ChIP-seq marks, Hi-C contact maps, splice junction usage).
...
In contrast, models like AlphaGenome and Evo 2 are generalist backbones that can be adapted across many contexts via multi-task heads or fine-tuning. Generalist models provide a single representation that can be reused across tasks (@sec-fm-principles), enable transfer to cell types or assays with limited data, and support unified analyses that integrate multiple regulatory modalities.
...
A key challenge is to avoid overcounting correlated signals (e.g., multiple scores derived from similar evolutionary data) and to calibrate evidence contributions within frameworks like ACMG, as discussed further in Chapters 17 and 18 (@sec-eval; @sec-confounding).
