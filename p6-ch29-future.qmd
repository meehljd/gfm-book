...
Early cross-modal models (including systems like Omni-DNA, G2PT, and related architectures discussed in Chapters @sec-dna-lm, @sec-rna, and @sec-multi-omics) hint at what is possible, but they remain limited in scope, scale, and biological coverage. Open questions include how to:
...
Answers will likely involve layered transparency. At one layer, model cards, data statements, and benchmark reports provide technical documentation. At another, user-facing interfaces convey uncertainty, intended use, and limitations in accessible language. Interpretability tools from @sec-interpretability can help connect internal model behavior to human-understandable patterns, though they are not sufficient on their own.
