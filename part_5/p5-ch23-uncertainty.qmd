...
## Calibration and Confidence Interpretation {#sec-calibration}
...
[High] Three-column comparison with before/after reliability diagrams. Column 1 (Temperature scaling): Formula $p = \text{softmax}(z/T)$; single parameter $T$; before/after reliability showing improvement; annotation "Preserves ranking; cannot fix complex patterns." Column 2 (Platt scaling): Formula $p = \sigma(az + b)$; two parameters $a$ and $b$; handles bias and confidence adjustment; annotation "Assumes logistic relationship." Column 3 (Isotonic regression): Non-parametric monotonic fit shown as step function; flexible with no distributional assumptions; annotation "Overfits with limited data." Bottom panel: Decision guide flowchart for method selection based on calibration set size and miscalibration complexity.
:::

### Temperature Scaling

The simplest calibration fix is often the most effective. Temperature scaling applies a single learned parameter to adjust model confidence, dramatically improving calibration with minimal computational overhead and no change to predictions' ranking.
...
The choice among calibration methods depends on calibration set size and the complexity of miscalibration patterns.

For large calibration sets (thousands of examples) with complex miscalibration patterns visible in reliability diagrams, isotonic regression provides the flexibility to correct arbitrary monotonic distortions. The risk of overfitting diminishes with sufficient data, and the non-parametric nature handles patterns that parametric methods cannot capture.

For moderate calibration sets (hundreds of examples) or when miscalibration appears roughly linear in log-odds space, Platt scaling offers a good balance between flexibility and sample efficiency. The two parameters can correct both location and scale miscalibration without the variance that isotonic regression exhibits with limited data.

For small calibration sets (fewer than a hundred examples) or when the primary issue is simple overconfidence without complex distortion, temperature scaling provides the most robust correction. The single parameter is unlikely to overfit even with very limited calibration data.
