...
On one side, sequencing has become routine. Large biobanks, rare disease programs, and functional genomics consortia now generate data at a scale that would have been hard to imagine a decade ago. Whole genomes and exomes are becoming standard of care in some clinical settings, and single-cell assays and perturbation screens are increasingly common in research labs.

On the other side, deep learning has matured. Convolutional networks, transformers, and related architectures have already transformed how we represent language, images, and proteins. Self-supervised learning, large-scale pretraining, and the “foundation model” mindset are now part of the everyday vocabulary of many machine learning teams.

This book is about the intersection: **genomic foundation models**—large, learned sequence representations that aim to be reused across many tasks, from variant interpretation to clinical risk prediction.

Rather than offering a general introduction to either genomics or machine learning, the goal here is narrower and more opinionated:

> To provide a conceptual and practical map of how modern deep learning models are used in genomics, what makes them succeed or fail, and how they can be used responsibly in research and clinical workflows.

The chapters trace a path from classic pipelines and pre-deep-learning methods, through CNN-based sequence-to-function models and transformers, to multi-omic GFMs and real-world applications. Along the way, we will repeatedly ask:

- What information is actually present in the data?  
- What inductive biases do different architectures impose?  
- How should we evaluate these models in ways that connect to biological and clinical decisions?  
...
In genomics, this paradigm is still evolving and far from settled. Some tasks benefit dramatically from pretraining; others barely move beyond strong classical baselines. A central theme of the book is learning to tell the difference.
...
The book is organized into six parts plus two short appendices. Each part can be read on its own, but they are designed to build on one another. :contentReference[oaicite:1]{index=1}  
...
- **Chapter 2 — Foundational Genomics Data**  
  Surveys the core data resources that underlie most modern work—reference genomes, population variation catalogs, clinical variant databases, and functional genomics consortia—and discusses how they are used as training targets and evaluation benchmarks.
...
- **Chapter 17 — Interpretability & Mechanisms**  
  Explores interpretability tools from classical motif discovery and attribution methods to emerging mechanistic approaches, and asks when these tools genuinely reveal biological mechanism.
...
- **Appendix A — Deep Learning Primer for Genomics**  
  A compact introduction to neural networks, CNNs, transformers, training, and evaluation, aimed at genomics-first readers who want enough ML background to engage with the main chapters. :contentReference[oaicite:2]{index=2}  
