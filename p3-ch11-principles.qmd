...
Scaling laws
Multi-species vs intra-species
(@consens_transformers_2023)
- **Critique of MLM/ALM as “imported” pretext tasks**  
  The review explicitly argues that **MLM and ALM were imported from NLP with little biological tailoring**, and may not be the right pretraining objectives for genomics. They point to:
  - Self-GenomeNet as an example of a biologically motivated pretext that uses reverse complements and neighboring sequence structure.  
  - Results showing random k-mer embeddings can perform comparably to biologically pre-trained embeddings on some downstream tasks, questioning how much genomic grammar current pretraining actually learns. :contentReference[oaicite:7]{index=7}  
  This is *perfect* material for a short, critical subsection in p3-ch11-principles (“How much does pretraining really help?”).

(@ma_bingo-large_2023)
Ma et al. apply FGM-style adversarial perturbations to internal GNN embeddings (not raw sequence) and show:
small performance gains,
but noticeably smoother and lower training loss, suggesting reduced overfitting (Figure 4F).
:::
