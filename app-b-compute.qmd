...
These issues are developed in more depth in the evaluation and confounding chapters (@sec-eval and @sec-confounding).
...
While the main book has dedicated chapters for evaluation (@sec-eval), confounding (@sec-confounding), and clinical metrics (@sec-clinical-risk), it’s useful to have a few basic concepts in mind.
...
**Data leakage** occurs when information from the test set “leaks” into training (e.g., through overlapping loci or related individuals), leading to overly optimistic estimates of performance. @sec-eval and @sec-confounding discuss strategies for leak-resistant splits in detail.
