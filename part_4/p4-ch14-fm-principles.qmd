...
*AlphaMissense* applies protein language models to predict pathogenicity of missense variants [@cheng_alphamissense_2023] (@sec-ch18-alphamissense). *ESM-1v* uses evolutionary context for protein variant effect prediction (@sec-ch15-zeroshot-plm). *EVE* combines evolutionary and structural information (@sec-ch15-alignment-models). Genomic foundation models like *DNABERT* and *Enformer* provide variant effect predictions through *in silico* mutagenesis (@sec-ch15-dna-vep). The architecture, training, evaluation, and clinical deployment of variant effect predictors are covered comprehensively in @sec-ch18-vep-fm, with integration into clinical workflows detailed in @sec-ch29-rare-disease.
