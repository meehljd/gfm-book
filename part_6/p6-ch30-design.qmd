...
![**FIGURE PLACEHOLDER**](../figs/part_6/ch28/07-fig-generative-evaluation.png)

[High] Multi-dimensional evaluation framework. Panel A (Metric Landscape): Four axes radar chart showing novelty, validity, diversity, functionality; ideal designs occupy high values on all axes; trade-offs between axes. Panel B (Computational Metrics): Perplexity histogram comparing generated vs natural sequences; novelty: nearest-neighbor distance distribution; diversity: pairwise distance matrix within generated set. Panel C (Validity Pipeline): Generated sequence → structure prediction (folds? pLDDT > 70?) → constraint check (GC content, length, forbidden motifs) → passes to experimental testing. Panel D (Experimental Validation): Funnel showing attrition: 1000 generated → 500 valid → 100 synthesized → 30 express → 5 functional; success rate varies by task and model quality.
:::
