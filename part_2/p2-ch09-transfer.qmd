...
Ji et al. demonstrated that *DNABERT* embeddings paired with linear probes achieve competitive chromatin accessibility prediction from a few hundred positive and negative examples, matching **convolutional neural network (CNN)** baselines requiring far more labeled data [@ji_dnabert_2021]. Dalla-Torre et al. showed similar results with *Nucleotide Transformer*, where linear probes on frozen embeddings approached fine-tuned performance for promoter detection and splice site recognition [@dalla-torre_nucleotide_2023]. These successes reflect alignment between pretraining objectives (predicting masked tokens from local context) and target tasks (distinguishing sequences based on motif patterns the model already learned to recognize).
...
A clinical genomics team has decided on LoRA adaptation for their tissue-specific expression predictor. The conceptual choice is made, but practical questions remain. What rank should the low-rank matrices use? Should adapters target attention weights, feed-forward layers, or both? Which transformer layers matter most for their specific task? These implementation decisions can mean the difference between successful adaptation and wasted compute.

**Rank selection** balances expressiveness against overfitting risk. Ranks of 4 to 16 suffice for tasks closely aligned with pretraining objectives, where small perturbations to pretrained weights capture the required adaptation. Ranks of 32 to 64 may be necessary when target tasks diverge more substantially, though higher ranks approach the parameter count where full fine-tuning becomes competitive. Empirical comparison across ranks on held-out validation data remains the most reliable selection method; theoretical guidance for optimal rank given task characteristics does not yet exist.

**Layer selection** depends critically on whether the foundation model uses encoder or decoder architecture. Encoder models like *DNABERT* and *Nucleotide Transformer* process entire sequences bidirectionally, building representations that integrate context from both directions at every layer. For these models, middle and later layers typically encode the most task-relevant features: early layers capture local sequence patterns (motifs, k-mer statistics) while deeper layers integrate these into higher-order representations. Adapting only the final third of transformer layers often achieves most of the performance gain at a fraction of the parameter cost. Linear probing experiments across layers can identify where task-relevant information concentrates before committing to adapter placement.
...
## Minimal-Data and Emerging Transfer Paradigms
...
## Diagnosing Transfer: Validation and Failure Modes
