...
The transformer architecture changed sequence modeling by replacing recurrence with attention, enabling parallel processing and capturing long-range dependencies without vanishing gradient issues [@vaswani_attention_2023]. As Paass and Giesselbach describe in *Foundation Models for Natural Language Processing* [@paass_foundation_2023], this architectural innovation matters for genomic sequences because regulatory relationships span thousands to millions of base pairs—distances that challenged earlier recurrent and convolutional approaches. The self-attention formulation has specific properties important for genomic modeling: **permutation equivariance** (without positional encoding, attention is position-agnostic), **quadratic complexity** (computation scales as O(L²) in sequence length), and **global receptive field** (every position has the capacity to attend to every other position, providing the potential to model distal regulatory relationships). The first property necessitates positional encodings for sequence-sensitive tasks; the second limits context length and motivates efficient attention variants (discussed in @sec-ch07-efficient-attention); the third enables modeling of distal regulatory relationships given appropriate training signals.
