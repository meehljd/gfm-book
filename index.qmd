...
By contrast, the **foundation model** paradigm rests on three linked ideas: large-scale pretraining on broad data, self-supervised objectives, and reuse of a shared backbone across many downstream tasks [@bommasani_foundation_2022]. In biomedicine and bioinformatics, this paradigm is increasingly being adapted to molecular, cellular, and clinical data [@wang_crisis_2025; @liu_foundation_2025; @guo_foundation_2025].
...
2. **Self-supervision**  
   Use objectives such as masked-token prediction, next-token modeling, or multi-task sequence-to-function training that do not require dense human labels (e.g., @benegas_gpn_2023; @zvyagin_genslms_2022; @nguyen_hyenadna_2023; @schiff_caduceus_2024).

3. **Reusability**  
   Treat the model as a *backbone*: for any new task, you probe or fine-tune it rather than training from scratch. Variant effect prediction, enhancer classification, GWAS fine-mapping priors, and clinical risk models all become “adapters” on top of a shared representation (e.g., @benegas_genomic_2024; @fishman_gena-lm_2025).
...
- **Chapter 14 — Multi-omics and Systems Context**  
