...
# Deployment and Compute {#sec-apx-compute}
...
These issues are developed in more depth in the evaluation and confounding chapters (@sec-evaluation and @sec-confounding).
...
The model and application chapters provide details about which metrics are appropriate for which tasks.  See @sec-evaluation for more on evaluation metrics.
...
@sec-dna-lm generalizes this story to broader DNA foundation models, including alternative tokenization schemes and architectures.
...
While the main book has dedicated chapters for evaluation (@sec-evaluation), confounding (@sec-confounding), and clinical metrics (@sec-clinical-risk), it’s useful to have a few basic concepts in mind.
...
**Data leakage** occurs when information from the test set “leaks” into training (e.g., through overlapping loci or related individuals), leading to overly optimistic estimates of performance. @sec-evaluation and @sec-confounding discuss strategies for leak-resistant splits in detail.
...
2. **Choose an input representation**
   - One-hot encoding or tokenization scheme for sequences (see @sec-representations).  
...
7. **Inspect and stress-test**
   ...
   - Use interpretability tools (@sec-interpretability) to see what patterns the model is using.  
