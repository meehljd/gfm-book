...
$$\hat{R}_{train}(\hat{f}) = \frac{1}{n} \sum_{i=1}^n L(y_i, \hat{f}(x_i))$$ {#eq-training-error}

**Goal.** Bound the *generalization error*:

$$R(\hat{f}) = \mathbb{E}_{(x,y) \sim P}[L(y, \hat{f}(x))]$$ {#eq-generalization-error}
...
$$R(\hat{f}) \leq \hat{R}_{train}(\hat{f}) + \sqrt{\frac{d_{VC} (\log(2n/d_{VC}) + 1) + \log(4/\delta)}{n}}$$ {#eq-vc-bound}
...
$$\mathfrak{R}_n(\mathcal{H}) = \mathbb{E}\left[\sup_{h \in \mathcal{H}} \frac{1}{n} \sum_{i=1}^n \sigma_i h(x_i)\right]$$ {#eq-rademacher}

where $\sigma_i \in \{-1, +1\}$ are random signs.

With probability $1 - \delta$:
$$R(\hat{f}) \leq \hat{R}_{train}(\hat{f}) + 2\mathfrak{R}_n(\mathcal{H}) + \sqrt{\frac{\log(1/\delta)}{2n}}$$ {#eq-rademacher-bound}
...
$$R(\hat{f}) \leq \hat{R}_{train}(\hat{f}) + \sqrt{\frac{KL(Q || P) + \log(n/\delta)}{2n}}$$ {#eq-pac-bayes}
...
**Chinchilla Law:**
$$L(N, D) = A \cdot N^{-\alpha} + B \cdot D^{-\beta} + L_\infty$$ {#eq-chinchilla}
