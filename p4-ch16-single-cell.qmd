...
The three-dimensional organization of the genome, examined in @sec-3d-genome, provides crucial context for interpreting cellular state. Chromatin accessibility and gene expression are not independent of genome folding; enhancer-promoter contacts determine which regulatory elements can influence which genes. Models that integrate 3D structure with cellular state measurements may achieve more accurate predictions than either approach alone.  Spatial transcriptomics (@sec-3d-genome) adds positional context that reveals how cellular state varies across tissue architecture, connecting expression profiles to physical location.
